  ...
  <div class="m-colorPicker" ref="colorPicker" v-on:click="event => { event.stopPropagation() }">
    ...
    <div class="box" v-bind:class="{ open: openStatus }">
      ...
      <div class="bd">
        ...
        <h3 v-on:click="triggerHtml5Color" style="display:none;">More Colors...</h3>
      </div>
    </div>
  </div>
</template>
...
.m-colorPicker{
  ...
  input{ display: none; }
  .colorBtn{ width: 15px; height: 15px; margin-top:-1px; }
  .colorBtn.disabled{ cursor: no-drop; }
  .box{
    position: absolute; width: 190px; background: #fff; border: 1px solid #ddd; visibility: hidden; border-radius: 2px; margin-top: 2px; padding: 10px; padding-bottom: 5px; box-shadow: 0 0 5px rgba(0,0,0,.15); opacity: 0; transition: all .3s ease; z-index: 999999;
